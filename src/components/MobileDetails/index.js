...
export default function MobileDetails() {
  ...
  const mutation = useMutation(newProduct => {
    fetch(`https://front-test-api.herokuapp.com/api/cart`, { method: 'POST', body: JSON.stringify(newProduct) }).then(res =>
  ...
  const handleAddToCart = async () => {
    await mutation.mutate({ id, colorCode: parseInt(selectedColor), storageCode: parseInt(selectedStorage) })
  }
    ...
    <div className="show-product">
      <div className="container">
        <div className="container-2">
          ...
          <button className="btncl" onClick={handleAddToCart} >Add to Cart</button>
        </div>
